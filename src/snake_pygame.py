...
Run:          python src/snake_pygame.py
"""
...
BASE_SPEED = 5             # ticks per second at start
...
class SnakeGame:
    def __init__(self, seed=None):
        ...
        if seed is None:
            seed_input = input("Enter seed (press Enter for default): ").strip()
    ...
    def _draw_hud(self):
        txt = self.font_sm.render(
            f"score {self.score}   high {self.high}   speed {self.speed}   seed {self.seed}",
            True, C_MUTED)
